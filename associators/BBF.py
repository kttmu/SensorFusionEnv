...
class BBF:
	def __init__(self):
		...
		self.xhi_val = 13.3 #if obsed dim == 6, set 16.8 or 18.6 REF:http://www3.u-toyama.ac.jp/kkarato/2019/statistics/table/chisq.pdf 
	...
	def compute(self, track_obj, obs_obj={}, obs_cov=None, mode = "update"):
		...
		if mode == "measurement":
			...
			for s_id in obs_obj:
				for obs_id in range(len(obs_obj[s_id])):
					if not (det[s_id] == mini_cov and obs_id == 0):
						#print("sense!")
						z = obs_obj[s_id][obs_id].state
						d = (z - zc).T @ inv(obs_cov[det[s_id]]) @ (z - zc)
						if d > self.xhi_val:
							d = 100
			...
			if "fused" in obs_obj:
				...
				for s_id in obs_obj:
					for i in range(len(obs_obj[s_id])):
						...
						p = 0.65
						L_t = np.log(p / (1 - p))
			L = track_obj.tracker.L + L_t - 0.3# + L_obs
			P = float(np.exp(L) / (1 + np.exp(L)))
			track_obj.tracker.prob = P
			print(track_obj.tracker.L)
			track_obj.tracker.L = L
			print("prob;",P)
